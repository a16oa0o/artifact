...
REPO_ROOT=/root/artifact/submodules/MOM6-tuning

FMS_BLD_DIR=${REPO_ROOT}/build/gnu/shared/gptl
MOM6_BLD_DIR=${REPO_ROOT}/build/gnu/ocean_only/gptl
...
if test -f "$FMS_BLD_DIR/libfms.a"; then
  ...
  make clean
  make NETCDF=3 REPRO=1 GPTL=1 libfms.a -j #(this flag is from the wiki instructions for ubuntu...)
fi
...
../../../../src/mkmf/bin/mkmf -t $GNU_MKMF_TEMPLATE -o '-I../../shared/gptl' -p 'MOM6 -L../../shared/gptl -lfms' -c '-Duse_libMPI -Duse_netCDF -DSPMD' path_names
# command taken from wiki instructions for ubuntu; removed -l flag, moved MOM6 inside quotes
make NETCDF=3 REPRO=1 GPTL=1 $TARGET_MOM6_OBJ -j #(this flag is from the wiki instructions for ubuntu...)
...
echo "Successfully built MOM6 with GNU and flags for GPTL."
