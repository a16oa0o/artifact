...
FMS_BLD_DIR=${REPO_ROOT}/build/gnu/shared/repro
MOM6_BLD_DIR=${REPO_ROOT}/build/gnu/ocean_only/repro
...
if test -f "$FMS_BLD_DIR/libfms.a"; then
  ...
  make clean
  make NETCDF=3 REPRO=1 libfms.a -j #(this flag is from the wiki instructions for ubuntu...)
fi
...
../../../../src/mkmf/bin/mkmf -t $GNU_MKMF_TEMPLATE -o '-I../../shared/repro' -p 'MOM6 -L../../shared/repro -lfms' -c '-Duse_libMPI -Duse_netCDF -DSPMD' path_names
