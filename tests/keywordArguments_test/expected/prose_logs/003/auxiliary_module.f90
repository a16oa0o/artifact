...
IMPLICIT NONE
public right_wrapper_84x_to_880
INTERFACE right_wrapper_84x_to_880
MODULE PROCEDURE right_wrapper_84x_to_880
END INTERFACE 
...
SUBROUTINE right_wrapper_84x_to_880(result,multiplicand)
REAL(kind=8) :: result
REAL(kind=4), OPTIONAL :: multiplicand
REAL(kind=8), POINTER :: multiplicand_temp
IF (PRESENT(multiplicand)) THEN
...
multiplicand_temp => NULL()
END IF
CALL right(result=result,multiplicand=multiplicand_temp)
IF (PRESENT(multiplicand)) THEN
multiplicand = multiplicand_temp
deallocate( multiplicand_temp )
END IF
