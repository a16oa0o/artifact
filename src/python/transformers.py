...
class ProseProjectTransformer:
        ...
        while configuration_dict:
            ...
            if self.timeout < 0:
                if self.SETUP['run']['timeout'] == '0':
                    self.timeout = int(np.ceil(total_cost * 1.5))
                else:
    ...
    def report(self, final=False):
        ...
        failed_costs = []
        passed_costs = []
        for message_file in sorted(os.listdir("./prose_workspace/__search_progress"), key=lambda x: int(x)):
            with open(os.path.join("./prose_workspace/__search_progress", message_file), "r") as f:
    ...
    def _evaluate(self, working_dir, configuration_dir):
        ...
        if len(glob(os.path.join(working_dir, "timing.*"))) == 0:
            ...
            subprocess.run(f"tar -czf {configuration_dir}/gptl_timing.tar.gz timing.* && rm timing.*", shell=True, cwd=working_dir)
        ...
        while batch:
            ...
            for i in range(len(batch)):
                if self.timeout < 0: # should only happen on the default (first) configuration
                    if self.SETUP['run']['timeout'] == '0':
                        try:
                            self.timeout = int(np.ceil(costs[0][0] * 1.5))
